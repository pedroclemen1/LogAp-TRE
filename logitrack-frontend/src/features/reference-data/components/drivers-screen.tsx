...
import type { Driver, ReferenceFilters } from '../model/reference-data'
...
export function DriversScreen({ drivers, filters, canManage }: {
  ...
  const [formTarget, setFormTarget] = useState<Driver | null>()
  const [deactivateTarget, setDeactivateTarget] = useState<Driver>()
  ...
  function reactivate(driver: Driver) {
    ...
    startTransition(async () => {
      ...
      if (result.status === 'error') {
        ...
        router.refresh()
        showToast({ tone: 'success', title: t('reactivatedTitle'), description: t('reactivatedDescription', { name: driver.name }) })
      }
    })
  }
    ...
    <div className="space-y-5 lg:space-y-6">
      ...
      <Surface className="overflow-hidden shadow-sm">
            ...
            <TableBody className="divide-y divide-outline-variant/50 bg-surface">
              ...
              {drivers.map((driver) => (
                <TableRow key={driver.id} className="h-compact-row-height hover:bg-surface-container-low">
                  <TableCell className="px-4 py-2 font-medium text-on-surface">{driver.name}</TableCell>
        ...
        <div className="border-t border-outline-variant bg-surface-container-lowest px-4 py-3 text-body-sm text-on-surface-variant">
          {t('displayed', { count: drivers.length })}
        </div>
      </Surface>
